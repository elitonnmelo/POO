...
public:
    ...
    void enter() {
      ...
      PtrKid auxi = waiting.back();
      waiting.pop_back();
      this->playing.push_front(auxi);
    }
    ...
    void leave() {
      ...
      PtrKid auxi = playing.back();
      playing.pop_back();
      this->waiting.push_front(auxi);
    }

    PtrKid removeKid(std::string name) {
      if(removeFromList( name, playing) != nullptr){
        return removeFromList( name, playing);
      }
      else{
      return removeFromList( name, waiting);
      }
    }
    std::string str() {
